...
# MPI Benchmark done by Enzo Peigné
...
echo "input,cores,run1,run2,run3,run4,run5" > runtime.csv
...
do
    ...
    do
        ...
        do
            ...
            # printf "%s" "$(mpirun -n 4 ./totientMPI 1 $inputSize)" >> runtime.csv
            module load intel/mpi/64
	    # to test on a Robotarium cluster compute node
            printf "%s" "$(srun --partition=amd-longq --mpi=pmi2  -N 3 -n $cores ./totientMPI 1 $inputSize)" >> runtime.csv
        done
        echo "" >> runtime.csv
    done
done
